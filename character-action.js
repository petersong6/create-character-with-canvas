...
var char_res={
	'imgs': ['leftArm','legs','torso','rightArm','head','hair','leftArm-jump','rightArm-jump','legs-jump'],
	'images': {},
	...
	'resourceLoaded': function(canvas_width, canvas_height){
		...
		if(char_res.numResourcesLoaded === char_res.imgs.length) {
			...
			$('#canvasDiv').mousedown(function(){	//jump action
				if(!char_res.isJumping){
					char_res.isJumping=true;
					setTimeout(function(){ char_res.isJumping=false; }, 500);	//land action
				}
			});
	...
	'redraw': function(x, y){
		...
		var jumpHeight = 45;	//jump 高度

		canvas.width = canvas.width; // clears the canvas

		if(char_res.isJumping){		//Draw shadow
			this.drawEllipse(context, x + 40, y + 29, 100, 4);
		}else{
		...
		if(char_res.isJumping){
			context.drawImage(this.images['leftArm-jump'], x + 40, y - 42 - char_res.breathAmt);
		}else{
			context.drawImage(this.images['leftArm'], x + 40, y - 42 - char_res.breathAmt);
		}
		if(char_res.isJumping){
			context.drawImage(this.images['legs-jump'], x, y - 6);
		}else{
			context.drawImage(this.images['legs'], x, y);
		}
		context.drawImage(this.images['torso'], x, y -50);
		if(char_res.isJumping){
			context.drawImage(this.images['rightArm-jump'], x - 35, y - 42 - char_res.breathAmt);
		}else{
